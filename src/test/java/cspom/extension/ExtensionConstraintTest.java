...
public class ExtensionConstraintTest {
    ...
    public void testExtensionConstraint() {
        ...
        ExtensionConstraint constraint = new ExtensionConstraint("c1",
                relation, v1, v2, v3);

        assertEquals(
                "c1 ([v1 (d1: [6]), v2 (d1: [6]), v3 (d1: [6])]): test: 3-ary, 4 tuples, supports",
                constraint.toString());
        assertEquals("1 2 3|2 3 4|3 4 5|4 5 6", constraint.getRelation()
                .tupleString());

        ExtensionConstraint c2 = constraint.standardize(new Variable[] { v2,
                v3, v1 });

        assertEquals(
                "c1 ([v2 (d1: [6]), v3 (d1: [6]), v1 (d1: [6])]): test: 3-ary, 4 tuples, supports",
                c2.toString());
