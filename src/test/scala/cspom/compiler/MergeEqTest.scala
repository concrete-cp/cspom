...
class MergeEqTest {
  ...
  def test1() {
    val problem = CSPOM {
      val v1 = IntVariable.ofInterval(0, 10, Map("output_var" -> Unit)) as "V1"
  ...
  def testExt() {
    ...
    val cspom = CSPOM {
      val v0 = IntVariable.of(1, 2, 3) as "V0"
      ctr(CSPOMConstraint('dummy, v0))
      val v1 = IntVariable.ofSeq(Seq(2, 3, 4), Map("var_is_introduced" -> Unit))
      ctr(v0 === v1)
    }
