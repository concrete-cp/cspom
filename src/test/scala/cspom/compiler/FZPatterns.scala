...
object FZPatterns {
  ...
  def linear[A: TypeTag](vars: Seq[SimpleExpression[A]], coefs: Seq[Int], mode: String, constant: Int): CSPOMConstraint[Boolean] =
    CSPOMConstraint("sum")(coefs.map(CSPOMConstant(_)), vars, CSPOMConstant(constant)) withParam ("mode" -> mode)

  def apply(): Seq[ConstraintCompiler] = Seq[(String, PartialFunction[CSPOMConstraint[_], CSPOMConstraint[_]])](
    /**
    ...
    /**
      ...
      */
    "bool_le_reif" -> { case Ctr(_, Seq(a, b, r)
    , p
