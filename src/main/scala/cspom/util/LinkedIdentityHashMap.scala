...
class LinkedIdentityHashMap[A <: AnyRef, B] extends
  mutable.AbstractMap[A, B] { //} with collection.mutable.Map[A, B] with collection.mutable.MapOps[A, B, mutable.Map, LinkedIdentityHashMap[A, B]] {

  override def empty = new LinkedIdentityHashMap()

  private val map = new mutable.LinkedHashMap[IdentityWrapper[A], B]()

  override def addOne(kv: (A, B)): LinkedIdentityHashMap.this.type = {
    map += new IdentityWrapper(kv._1) -> kv._2
    this
  }

  override def subtractOne(key: A): LinkedIdentityHashMap.this.type = {
