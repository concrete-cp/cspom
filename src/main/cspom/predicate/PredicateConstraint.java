...
public class PredicateConstraint extends AbstractConstraint {
	...
	public PredicateConstraint(String name, List<Variable> scope,
		...
		this.parametersNames = new HashMap<String, Parameter>(nbParameters);

		this.varPointers = new ArrayList<Pointer>(getArity());

		for (int i = getArity(); --i >= 0;) {
			this.varPointers.add(new Pointer(scope.get(i)));
		}
		...
		for (Parameter p : parameters) {
			...
			if (p instanceof ConstantParameter) {
				...
				this.pointers.add(varPointers
						.get(seekVariable(((VariableParameter) p).getVariable()
								.getName(), scope)));
			}
		}
	...
	public PredicateConstraint(String name, List<Variable> scope,
			String parameters, Predicate predicate) {
		this(name, scope, parseParameters(parameters, predicate, scope),
				predicate);

	}

	private static List<Parameter> parseParameters(final String allParameters,
			final Predicate predicate, final List<Variable> scope) {
		final List<Parameter> parameters = new ArrayList<Parameter>();

		final String[] stringParameters = allParameters.trim().split(" +");

		for (int i = 0; i < stringParameters.length; i++) {
			final String parameterName = predicate.getParameters().get(i);

			if (CONSTANT_PATTERN.matcher(stringParameters[i]).matches()) {
				parameters.add(new ConstantParameter(parameterName, Integer
						.parseInt(stringParameters[i])));
			} else {
				final int variable = seekVariable(stringParameters[i], scope);

				if (variable >= 0) {
					parameters.add(new VariableParameter(parameterName, scope
							.get(variable)));
				} else {
					throw new IllegalArgumentException(
							"Could not find variable " + stringParameters[i]
									+ " " + scope);
				}
			}
		}
	...
	private static int seekVariable(final String name,
			final List<Variable> scope) {
		for (int i = scope.size(); --i >= 0;) {
			if (name.equals(scope.get(i).getName())) {
				return i;
			}
		}
		return -1;
	...
	public boolean evaluate(final Number[] numbers) throws ScriptException {
...
//		logger.finer("Semicompiling " + predicate);
		...
		for (int i = 0; i < varPointers.size(); i++) {
			newVars.add(new VariableParameter("mouk" + i, varPointers.get(i)
					.getVariable()));
		}
		...
		for (int p = this.parameters.size(); --p >= 0;) {
			...
			if (parameter instanceof ConstantParameter) {
				expression = expression.replaceAll(parameter.getName(),
						((ConstantParameter) parameter).getNumber().toString());
			} else if (parameter instanceof VariableParameter) {
				final int varnum = varPointers.indexOf(pointers.get(p));

				expression = expression.replaceAll(parameter.getName(), newVars
						.get(varnum).getName());
			}
		}
//		logger.finest("Expression : " + expression);
		final Map<String, Type> types = new HashMap<String, Type>(newVars
				.size());
		final List<String> stringParameters = new ArrayList<String>(newVars
				.size());
	...
	public PredicateConstraint standardize(final List<Variable> scope) {
		...
		return new PredicateConstraint(getName(), scope, newParameters,
				predicate);
	}
