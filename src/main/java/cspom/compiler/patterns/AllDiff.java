...
public final class AllDiff implements ConstraintCompiler {
	...
	public void alldiff(final CSPOMConstraint constraint) {
		...
		final Set<CSPOMVariable<?>> clique = new HashSet<CSPOMVariable<?>>(
				JavaConversions.seqAsJavaList(constraint.scope()));
	...
	private void populate(final Set<CSPOMVariable<?>> pool,
		...
		for (CSPOMConstraint c : JavaConversions.asJavaIterable(first
				.constraints())) {
			if (DIFF_CONSTRAINT.is(c)) {
				for (CSPOMVariable<?> n : JavaConversions.asJavaIterable(c
						.scope())) {
					if (n != first) {
						pool.add(n);
					}
	 ...
	 * @param scope
	 */
	public void newAllDiff(final Set<CSPOMVariable<?>> scope) {
		final CSPOMConstraint allDiff = new GeneralConstraint("allDifferent",
				null,
				JavaConversions.asScalaBuffer(new ArrayList<CSPOMVariable<?>>(
						scope)));
		...
		for (CSPOMVariable<?> v : scope) {
			for (CSPOMConstraint c : JavaConversions.asJavaIterable(v
					.constraints())) {
